...
import copy
from mmdet3d.models.vtransform.volume_transform import build_vtransform
from mmdet3d.ops import points_in_boxes_cpu,points_in_boxes_gpu
from mmdet3d.core import bbox3d2result, merge_aug_bboxes_3d
# torch.set_printoptions(profile="full")
eps=1e-9
@DETECTORS.register_module()
class RobustMAE(SingleStage3DDetector):
    ...
    def forward_train(self,
        ...
        if self.cls_sub_voxel:
            ...
            centroid_normal_low, centroid_normal_med, centroid_normal_high, all_camera_token, \
            camera_mask, camera_ids_restore \
                = self.extract_feat(points, gt_bboxes_3d, gt_labels_3d, img_metas, img)
        ...
        # img_decoder
        cam_pred = all_camera_token.view(B*N, -1, H//32, W//32)
        cam_pred = cam_pred.flatten(2).permute(0, 2, 1)
        cam_pred = self.camera_decoder(cam_pred, camera_ids_restore)
        img = img.view(B * N, C, H, W)
        loss['loss_cam'] = self.camera_decoder.forward_loss(img, cam_pred, camera_mask)
        return loss

    def extract_feat(self, points: list,gt_bboxes_3d,gt_labels_3d, img_metas, img, vis=False):
        ...
        # get voxel center points according to scatter function
        ...
        x, all_camera_token, camera_mask, camera_ids_restore = self.backbone(voxel_features[ids_keep],feature_coors[ids_keep],mask_coors,batch_size, img, img_metas, self.camera_encoder)

        if self.vis:
            return x, centroids_low, centroid_mask_low,centroids_med,centroid_mask_med,centroids_high,mask_coors[:,1:]
        else:
            return x, centroids_low, centroid_mask_low,centroids_med,centroid_mask_med,centroids_high,centroids_normal,centroids_normal_med,centroids_normal_high, all_camera_token, camera_mask, camera_ids_restore
    ...
    def get_vanilla_mask_index(self,coors,batch_size):
        ...
        ids_mask_list=[]
        for i in range(batch_size):
            inds = torch.where(coors[:, 0] == i) # [row, cloumn]
            L=inds[0].shape[0] # num_row
            len_keep = int(L * (1 - self.random_mask_ratio)) 
            ids_shuffle=torch.randperm(L,device=device) # shuffled_row_numbers
