...
sub_voxel_size_low = (0.125, 0.125, 1)
sub_voxel_size_med = (0.25, 0.25, 2)
sub_voxel_size_top = (0.5, 0.5, 8)
sub_voxel_ratio_low = (8,4,4) # z,y,x
sub_voxel_ratio_med = (4,2,2) # z,y,x
input_voxel_ratio = (1,2,2)
grid_size=(1,200,200)    # z,y,x
...
model = dict(
    ...
    backbone=dict(
        type='MultiMAESSTSPChooseFusion',
        cls_sub_voxel=cls_sub_voxel,
    ...
    camera_backbone=dict(
        type='MAESwinEncoder',
        img_size=img_size,
        patch_size=4, # area that encodes latent variable (11264 patches)
        in_chans=3,
        embed_dim=96,
        depths=[2, 2, 6, 2],
        num_heads=[3, 6, 12, 24],
        window_size=7, # area that performs self-attention 
        # common configs
        mlp_ratio=4,
        qkv_bias=True,
        qk_scale=None,
